...
GET /api/sample
[
  { "id": 1, "message": "Hello World", "status": "ACTIVE", "updatedAt": null },
  { "id": 2, "message": "[URGENT] ASAP!", "status": "ACTIVE", "updatedAt": null }
]
...
*   **`Sample` (Domain Model)**:
    ...
    *   **`applyPatch(message, status)`**: PATCH 부분 업데이트 전용 메서드. `null`인 인자는 기존 값을 유지하며, 변경된 결과를 새 객체로 반환합니다.
