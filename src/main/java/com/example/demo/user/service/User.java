...
public class User {
    ...
    public User login() {
        ...
        return new User(this.id, this.loginId, this.nickname, this.passwordHash,
                this.status, now, this.createdAt, now);
    }

    public User delete() {
        return new User(this.id, this.loginId, this.nickname, this.passwordHash,
                "deleted", this.lastLoginAt, this.createdAt, LocalDateTime.now());
    }
}
